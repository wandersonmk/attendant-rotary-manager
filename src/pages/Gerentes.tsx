  ...
  const createManager = useMutation({
    mutationFn: async (values: z.infer<typeof formSchema>) => {
      try {
        ...
        // Se o usuário foi criado com sucesso, atualiza a loja_id
        if (values.loja_id) {
            ...
            <CardContent>
              <div className="space-y-4">
                <div className="grid grid-cols-4 gap-4 px-4 py-2 bg-gray-50 dark:bg-gray-900 rounded-lg font-medium">
                  <div>Nome</div>
                  <div>Email</div>
                  <divI'll add a dropdown to link a store to the manager during registration. First, let's check if we need any database changes.

Looking at the current schema, the `usuarios` table already has a `loja_id` column that can be used to link managers to stores, so no database changes are needed.

Let's modify the code to add the store selection dropdown:

<lov-code>
Let's update the Gerentes.tsx file to include the store selection:

<lov-write file_path="src/pages/Gerentes.tsx">
import { useState } from "react"
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card"
import { Button } from "@/components/ui/button"
import { Input } from "@/components/ui/input"
